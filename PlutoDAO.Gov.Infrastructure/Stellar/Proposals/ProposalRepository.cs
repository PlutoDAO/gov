...
namespace PlutoDAO.Gov.Infrastructure.Stellar.Proposals
{
    public class ProposalRepository : IProposalRepository
    {
        ...
        )
        {
            ...
            var asset = Asset.CreateNonNativeAsset(
                assetCode,
            ...
            var changeTrustLineOp = new ChangeTrustOperation.Builder(ChangeTrustAsset.Create(asset))
                ...
                .Build();
            var paymentOp = new PaymentOperation.Builder(
                proposalMicropaymentSenderKeyPair,
                asset,
                EncodingHelper.MaxTokens.ToString()
            )
                .SetSourceAccount(proposalMicropaymentReceiverKeyPair)
                .Build();
            txBuilder
        ...
        )
        {
            ...
            var claimClaimableBalanceOp = new ClaimClaimableBalanceOperation.Builder(
                balanceId
            ).Build();
        ...
        {
            var plutoDaoResultsPublicKey = KeyPair
            ...
            var proposalMicropaymentReceiverPublicKey = KeyPair
                ...
                .AccountId;
            var response = await _server.Payments
                .ForAccount(plutoDaoResultsPublicKey)
                .Limit(200)
                .Execute();
            while (response.Embedded.Records.Count != 0)
            {
                ...
                foreach (var record in paymentRecords)
                    if (
                        record.AssetCode == assetCode
                        && record.To == plutoDaoResultsPublicKey
                        && record.AssetIssuer == proposalMicropaymentReceiverPublicKey
                    )
